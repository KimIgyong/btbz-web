...
import { anonymizeIp } from '../common/request-ip';
...
export class EventsService {
  ...
  /** 기록 실패가 다운로드를 막지 않도록 예외를 삼킨다(FR-7). IP는 익명화 저장(GDPR). */
  async recordDownload(file: string, platform: string, version: string, ip: string, userAgent: string): Promise<void> {
    try {
      await this.downloads.save(
        this.downloads.create({ file, platform, version, ip: anonymizeIp(ip), userAgent }),
      );
    } catch (err) {
  ...
  async recordPageView(page: string, ip: string, userAgent: string): Promise<void> {
    try {
      await this.pageViews.save(this.pageViews.create({ page, ip: anonymizeIp(ip), userAgent }));
    } catch (err) {
